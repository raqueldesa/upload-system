...
const FiledInput = () => {
	...
	return (
      ...
      <Input 
        style={{width:"400px",height:"200px",backgroundColor:"red",color:"white"}} 
        type="file" 
        ...
        ></Input>
			...
			<div style={{ marginTop: "3rem" }}>
				{erroBool ? data.map((col,idx) => 
          <div key={idx}>data: {col.date} | open: {col.open} | high: {col.high} | low: {col.low} | close:{col.close}</div>
        ):
        <div>{error}</div>
      }
			</div>
